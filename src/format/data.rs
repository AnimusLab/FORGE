...
impl ForgeValue {
    pub fn from_json(v: &Value) -> Self {
        match v {
            ...
            Value::Number(n) => {
                if let Some(i) = n.as_i64() { ForgeValue::Integer(i) }
                else if let Some(f) = n.as_f64() { ForgeValue::Float(f) }
                else { ForgeValue::Null }
            }
    ...
    pub fn to_json(&self) -> Value {
        match self {
            ...
            ForgeValue::Float(f)   => Value::Number(serde_json::Number::from_f64(*f).unwrap_or(0.into())),
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        match self {
            ...
            ForgeValue::Integer(i) => buf.extend_from_slice(&i.to_le_bytes()),
            ForgeValue::Float(f)   => buf.extend_from_slice(&f.to_le_bytes()),
            ForgeValue::Boolean(b) => buf.push(if *b { 1 } else { 0 }),
            ForgeValue::Null       => {}
        }
        buf
    }
    ...
    pub fn from_bytes(buf: &[u8], pos: &mut usize) -> Result<Self, String> {
        if *pos >= buf.len() { return Err("Unexpected end of data buffer".to_string()); }
...
pub fn row_from_bytes(buf: &[u8], pos: &mut usize) -> Result<ForgeRow, String> {
    ...
    *pos += 4;
    ...
    for _ in 0..field_count {
        ...
        *pos += 2;
        let name = String::from_utf8(buf[*pos..*pos+name_len].to_vec()).map_err(|e| e.to_string())?;
        *pos += name_len;
